...
from superos import CMD_HELP, LOGS, TEMP_DOWNLOAD_DIRECTORY
...
async def download(target_file):
    """ For .dl command, download files to the superos's server. """
    await target_file.edit("Processing using superos server ( ◜‿◝ )♡")
...
async def upload(u_event):
    """ For .upload command, allows you to upload a file from the superos's server """
    await u_event.edit("Processing ...")
    input_str = u_event.pattern_match.group(1)
    if input_str in ("superos.session", "config.env"):
