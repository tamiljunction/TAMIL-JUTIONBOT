...
@LEGEND.on(admin_cmd(pattern=r"save(?: |$)([\s\S]*)", outgoing=True))
...
@LEGEND.on(events.NewMessage(incoming=True, func=lambda e: e.is_private))
...
@LEGEND.on(admin_cmd(pattern="elog ?(.*)"))
...
@LEGEND.on(admin_cmd(pattern="nlog ?(.*)"))
async def set_no_log_p_m(event):
    if Config.PM_LOGGR_BOT_API_ID is not None:
        event.pattern_match.group(1)
        chat = await event.get_chat()
